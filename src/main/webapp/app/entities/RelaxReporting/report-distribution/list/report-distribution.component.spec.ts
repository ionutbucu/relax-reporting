...
import { sampleWithRequiredData } from '../report-distribution.test-samples';
...
describe('ReportDistribution Management Component', () => {
  let comp: ReportDistributionComponent;
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [HttpClientTestingModule, ReportDistributionComponent],
      providers: [
        {
      ...
      .overrideTemplate(ReportDistributionComponent, '')
      .compileComponents();
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.reportDistributions?.[0]).toEqual(expect.objectContaining({ rid: 'ABC' }));
  });

  describe('trackRid', () => {
    it('Should forward to reportDistributionService', () => {
      const entity = { rid: 'ABC' };
      jest.spyOn(service, 'getReportDistributionIdentifier');
      const rid = comp.trackRid(0, entity);
      expect(service.getReportDistributionIdentifier).toHaveBeenCalledWith(entity);
