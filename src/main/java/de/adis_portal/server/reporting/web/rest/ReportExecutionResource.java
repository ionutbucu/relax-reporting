...
public class ReportExecutionResource {
    ...
    public ResponseEntity<ReportExecutionDTO> createReportExecution(@Valid @RequestBody ReportExecutionDTO reportExecutionDTO)
        ...
        if (reportExecutionRepository.existsById(reportExecutionDTO.getRid())) {
            throw new BadRequestAlertException("reportExecution already exists", ENTITY_NAME, "idexists");
        }
        reportExecutionDTO = reportExecutionService.save(reportExecutionDTO);
        return ResponseEntity.created(new URI("/api/report-executions/" + reportExecutionDTO.getRid()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, reportExecutionDTO.getRid()))
    ...
    public ResponseEntity<ReportExecutionDTO> updateReportExecution(
        ...
        if (!reportExecutionRepository.existsById(rid)) {
            throw new BadRequestAlertException("Entity not found", ENTITY_NAME, "idnotfound");
        }

        reportExecutionDTO = reportExecutionService.update(reportExecutionDTO);
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, reportExecutionDTO.getRid()))
            .body(reportExecutionDTO);
    }
    ...
    public ResponseEntity<ReportExecutionDTO> partialUpdateReportExecution(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, reportExecutionDTO.getRid())
        );
    }
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<ReportExecutionDTO> getReportExecution(@PathVariable("id") String id) {
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteReportExecution(@PathVariable("id") String id) {
