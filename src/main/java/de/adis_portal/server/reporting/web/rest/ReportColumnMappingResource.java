...
public class ReportColumnMappingResource {
    ...
    public ResponseEntity<ReportColumnMappingDTO> createReportColumnMapping(@RequestBody ReportColumnMappingDTO reportColumnMappingDTO)
        ...
        if (reportColumnMappingRepository.existsById(reportColumnMappingDTO.getRid())) {
            throw new BadRequestAlertException("reportColumnMapping already exists", ENTITY_NAME, "idexists");
        }
        reportColumnMappingDTO = reportColumnMappingService.save(reportColumnMappingDTO);
        return ResponseEntity.created(new URI("/api/report-column-mappings/" + reportColumnMappingDTO.getRid()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, reportColumnMappingDTO.getRid()))
    ...
    public ResponseEntity<ReportColumnMappingDTO> updateReportColumnMapping(
        ...
        if (!reportColumnMappingRepository.existsById(rid)) {
            throw new BadRequestAlertException("Entity not found", ENTITY_NAME, "idnotfound");
        }

        reportColumnMappingDTO = reportColumnMappingService.update(reportColumnMappingDTO);
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, reportColumnMappingDTO.getRid()))
            .body(reportColumnMappingDTO);
    }
    ...
    public ResponseEntity<ReportColumnMappingDTO> partialUpdateReportColumnMapping(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, reportColumnMappingDTO.getRid())
        );
    }
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<ReportColumnMappingDTO> getReportColumnMapping(@PathVariable("id") String id) {
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteReportColumnMapping(@PathVariable("id") String id) {
